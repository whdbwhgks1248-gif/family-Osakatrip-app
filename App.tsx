...
const App: React.FC = () => {
  ...
  /**
   * 2. 실시간 동기화 (Realtime Subscription)
   */
  useEffect(() => {
        ...
        (payload) => {
          ...
          if (newData) {
            ...
            if (dataString !== lastSavedRef.current) {
              ...
              setSouvenirs(newData.souvenirs || []);
  ...
  const fetchFamilyData = useCallback(async (id: string) => {
    ...
    try {
      ...
      if (data) {
        ...
        lastSavedRef.current = JSON.stringify({ e: data.expenses, s: data.souvenirs });
      } else {
        const initialExpenses: any[] = [];
        const initialSouvenirs: any[] = [];
        await supabase.from('family_state').upsert({
          family_id: cleanId,
          expenses: initialExpenses,
          souvenirs: initialSouvenirs
        });
        lastSavedRef.current = JSON.stringify({ e: initialExpenses, s: initialSouvenirs });
      }
    } catch (e) {
        ...
        <div className="bg-white p-10 rounded-[3.5rem] shadow-2xl border border-red-100 max-w-sm w-full text-center space-y-8">
          ...
          <div className="space-y-3">
            ...
            <p className="text-sm text-slate-500 leading-relaxed font-medium">
              Vercel 환경 변수 설정 후<br/>반드시 <b>Redeploy</b>를 해주세요.
            </p>
          </div>
          <button 
            onClick={() => window.location.reload()}
            className="w-full bg-[#1675F2] text-white py-5 rounded-2xl font-black shadow-xl"
          >
            새로고침
          </button>
        </div>
  ...
  if (isResetting) {
    return (
      <div className="fixed inset-0 bg-[#1675F2] flex flex-col items-center justify-center z-[2000] text-white gap-4">
        <Loader2 className="animate-spin text-[#F2E96D]" size={40} />
        <p className="font-black tracking-widest uppercase text-xs">연결 해제 중...</p>
      </div>
    );
  }
        ...
        <div className="fixed inset-0 z-[1000] bg-[#1675F2] flex items-center justify-center p-6 animate-in fade-in">
          <div className="bg-white w-full max-w-[360px] rounded-[3rem] p-10 shadow-2xl space-y-8 animate-in zoom-in-95">
            <div className="text-center space-y-3">
              <div className="w-16 h-16 bg-[#F2E96D] text-[#1675F2] rounded-3xl flex items-center justify-center mx-auto mb-2">
                <KeyRound size={32} />
              </div>
            ...
            <div className="space-y-4">
              <input 
                ...
                className="w-full bg-[#F1F2F0] border-none rounded-2xl px-6 py-5 text-center text-xl font-black uppercase text-[#1675F2] placeholder:text-slate-300"
                placeholder="코드 입력"
              />
              <button 
                onClick={() => handleSetFamilyId(tempCode)}
                disabled={!tempCode.trim()}
                className="w-full bg-[#1675F2] text-white py-5 rounded-2xl font-black shadow-xl active:scale-95 transition-all disabled:opacity-50"
              >
                여행 시작하기
              </button>
            </div>
            ...
            <div className="flex justify-between items-center h-full">
              ...
              <div className="flex items-center gap-2">
                ...
                <button 
                  ...
                  className="p-2.5 bg-slate-50 text-slate-400 border border-slate-100 rounded-full active:scale-90 transition-all"
                >
                  <Menu size={20}/>
                </button>
              </div>
          ...
          <main className="flex-1 px-4 pt-[118px] pb-32">
            {isLoading ? (
              <div className="flex flex-col items-center justify-center py-40 gap-4">
                ...
                <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">데이터 불러오는 중...</p>
              </div>
            ) : (
          ...
          <nav className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[calc(100%-32px)] max-w-[468px] bg-[#1675F2] rounded-full px-2 py-2 shadow-2xl z-[150] flex justify-between items-center border border-white/10 backdrop-blur-md">
            {[
              ...
              <button
                ...
                className={`flex items-center justify-center h-12 rounded-full transition-all duration-300 ${
                  activeTab === tab.id ? 'bg-[#F2E96D] text-[#1675F2] px-6' : 'text-white/50 w-12 hover:text-white'
                }`}
              >
            ...
            <div className="fixed inset-0 z-[200] bg-black/30 backdrop-blur-sm animate-in fade-in">
              <div className="absolute right-0 top-0 h-full w-[85%] bg-white p-10 flex flex-col animate-in slide-in-from-right duration-500 shadow-2xl">
                ...
                <div className="flex-1 space-y-6">
                  <div className="p-8 bg-[#F8F9FD] rounded-[2.5rem] border border-slate-100 space-y-4">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">현재 가족 코드</p>
                    <p className="text-3xl font-black text-[#1675F2] tracking-tighter uppercase">{familyId}</p>
                    <p className="text-xs font-bold text-slate-400 leading-relaxed">가족들에게 이 코드를 알려주면 모두 함께 실시간으로 일정을 관리할 수 있습니다.</p>
                  </div>

                  <button 
                    onClick={() => setShowResetConfirm(true)}
                    className="w-full py-5 bg-red-50 text-red-500 rounded-2xl text-sm font-black flex items-center justify-center gap-2 active:scale-95 transition-all"
                  >
                    <LogOut size={16} />
                    연결 해제 및 코드 변경
                  </button>
                  ...
                  {showResetConfirm && (
                    <div className="p-6 bg-red-500 rounded-3xl text-white space-y-4 animate-in slide-in-from-bottom-4">
                      <p className="text-xs font-bold leading-relaxed text-center">정말 연결을 해제할까요?<br/>기존 코드를 다시 입력하면 데이터는 유지됩니다.</p>
                      <div className="flex gap-2">
                        <button onClick={handleResetFamilyId} className="flex-1 py-3 bg-white text-red-500 rounded-xl text-[11px] font-black">해제하기</button>
                        <button onClick={() => setShowResetConfirm(false)} className="flex-1 py-3 bg-red-600 text-white rounded-xl text-[11px] font-black">취소</button>
                      </div>
                    </div>
                  )}
                </div>
