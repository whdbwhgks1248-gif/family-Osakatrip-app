  ...
  const memberSummaries = useMemo(() => {
    ...
    safeExpenses.forEach(exp => {
      ...
      exp.participantIds.forEach(pId => {
        ...
        if (pId !== exp.payerId) {
          const isSettled = Array.isArray(exp.settledMemberIds) && exp.settledMemberIds.includes(pId);
          if (isSettled) {
              ...
              <div key={member} className="flex items-center justify-between p-5 bg-[#F1F2F0] rounded-[2rem] border border-[#566873]/5 transition-all hover:bg-white hover:border-[#1675F2]/20">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-[#566873]/50 font-bold uppercase">₩{mSummary.spent.toLocaleString()}</p>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-start mb-4">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[9px] text-[#566873]/40 font-bold">{new Date(exp.date).toLocaleDateString('ko-KR', { month: 'short', day: 'numeric' })}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[11px] font-bold text-[#566873]/60">₩{exp.amount.toLocaleString()} ({exp.participantIds.length}인)</span>
                    </div>
                  </div>
